...
export function Header() {
  ...
  // If one wallet is connected, don't show the other option
  const showEvmButton = !suiAccount?.address
  const showSuiButton = !isEvmConnected
          ...
          {showEvmButton && (
            <ConnectKitButton.Custom>
              {({ isConnected, show, truncatedAddress }) => (
                ...
                >
                  {isConnected ? truncatedAddress : 'Client (EVM)'}
                </button>
              )}
            </ConnectKitButton.Custom>
          )}
          
          {/* Sui Wallet (for Freelancers) */}
          {showSuiButton && (
            <ConnectButton connectText="Freelancer (SUI)" />
          )}
        </div>
      </div>
    </header>
